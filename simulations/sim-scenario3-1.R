# simulation 3-1: method comparison given k under p = 20, Gaussian distribution and random CPC rankings
# environment setup ------------
set.seed(123)
...
ns <- 100
n_sim <- 1000
print("simulation 3-1: method comparison given k under p = 20, Gaussian distribution and random CPC rankings")
# parallelism
library(foreach)
library(doParallel)
cl <- makeCluster(32)
...
sim3.1 <- foreach(i=1:n_sim, .combine = cbind, .packages=c('tidyverse','pracma', 'multigroup', 'MASS')) %dopar% {
  # generate data
  ...
  sim_data <- map(sim_cov, ~MASS::mvrnorm(ns, rep(0,p), .))
...
saveRDS(sim3.1, "simulation3-1.rds")