# simulation 3-2: method comparison given k under p = 20, non-Gaussian distribution and
...
lambda_uncommon <- lambda[1:10]
lambda_common <- lambda[11:20]
n <- 100
ns <- 100
n_sim <- 1000
print("simulation 3-2: method comparison given k under p = 20, Gaussian distribution and fixed CPC rankings")
# parallelism
library(foreach)
library(doParallel)
cl <- makeCluster(32)
...
sim3.2 <- foreach(i=1:n_sim, .combine = cbind, .packages=c('tidyverse','pracma', 'multigroup', 'MASS')) %dopar% {
  # generate data
...
saveRDS(sim3.2, "simulation3-2.rds")