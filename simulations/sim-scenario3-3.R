...
ns <- 1000
n_sim <- 1000
print("simulation 2-3: method comparison given k under p = 100, Gaussian distribution and random CPC rankings")
# parallelism
library(foreach)
library(doParallel)
cl <- makeCluster(32)
...
sim2.3 <- foreach(i=1:n_sim, .combine = cbind, .packages=c('tidyverse','pracma', 'multigroup', 'MASS')) %dopar% {
  # generate data
...
saveRDS(sim2.3, "simulation2-3.rds")