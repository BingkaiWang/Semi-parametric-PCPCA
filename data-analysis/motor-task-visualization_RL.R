...
# plot of all CPC -------
png(file = "Motor-task/motor-task-plot-RL/taskSM-semi.png")
...
for(i in c(1:31)){
  ...
  roi_cpci <- filter(ROI.info.v2, module %in% c("SM_Hand", "SM_Mouth")) %>%
    ...
    mutate(color = ifelse(cpci > 0, 2, 1),
           size = ifelse(abs(cpci) > 0.1, abs(cpci), 0),
           label = '-') %>%
    filter(size != 0)
  write.table(roi_cpci,
              file = paste0("Motor-task/motor-task-plot-RL/cpc_node/taskSM", i, "_brainview.node"),
              col.names = F, row.names = F)
}

# visualize PC components ----------
...
p <- length(SM.indi)
ns <- nrow(ts.mc[[1]])
ts.mc.SM <- map(ts.mc, ~scale(.[,SM.indi]))
...
for(j in 1:31){
  ggplot() + 
    geom_line(aes(x = (1:ns)[-(1:3)], y = abs(avg_mat(ts.mc.SM) %*% cpc_taskSM[,j])[-(1:3)]), size = 1) +
  ...
  ggsave(paste0("data-analysis/figures/cpc_timecourse/taskSM-tc-cpc", j, ".jpg"))
}


# combing brainview and timecourse
...
cpc_error <- map_dbl(1:p, function(i){
  mean(map_dbl(reconstruct_taskSM_cor,
               ~sum(.[i,-i]^2/(eig_avg_taskSM_cor[i]*eig_avg_taskSM_cor[-i])))
  ) * ns / (p-1)
})
var_explained <- eig_avg_taskSM_cor/sum(eig_avg_taskSM_cor)
var_explained <- paste0(round(var_explained[order(cpc_error)], 3) * 100, '%')
for(j in 1:31){
  path1 <- paste0("data-analysis/figures/cpc_timecourse/taskSM-tc-cpc", j, ".jpg")
  path2 <- paste0("data-analysis/figures/cpc_brainview/taskSM-cpc", j, ".jpg")
  ...
  save_plot(paste0("data-analysis/figures/cpc_cmb/taskSM-cpc", j, ".jpg"), plot_tmp,
...
for(j in 1:ncol(ncpc_taskSM)){
  ggplot() + 
    geom_line(aes(x = (1:ns)[-(1:3)], y = abs(avg_mat(ts.mc.SM) %*% ncpc_taskSM[,j])[-(1:3)]), size = 1) +
    geom_rect(aes(xmin = tongue_start-3, xmax = tongue_end-3, fill = "tongue"), ymin = -Inf, ymax = Inf, alpha = 0.2) +
    geom_rect(aes(xmin = lf_start-3, xmax = lf_end-3, fill = "left foot"), ymin = -Inf, ymax = Inf, alpha = 0.2) +
    geom_rect(aes(xmin = rf_start-3, xmax = rf_end-3, fill = "right foot"), ymin = -Inf, ymax = Inf, alpha = 0.2) +
    geom_rect(aes(xmin = lh_start-3, xmax = lh_end-3, fill = "left hand"), ymin = -Inf, ymax = Inf, alpha = 0.2) +
    ...
    guides(fill = guide_legend(title = "Tasks", title.position = "left"))
  ggsave(paste0("data-analysis/figures/ncpc/taskSM-tc-ncpc", j, ".jpg"))
}


for(j in 1:ncol(ncpc_taskSM)){
  path1 <- paste0("data-analysis/figures/ncpc/taskSM-tc-ncpc", j, ".jpg")
  path2 <- paste0("data-analysis/figures/ncpc/taskSM-ncpc", j, ".jpg")
  plot_tmp <- plot_grid(ggdraw() + draw_image(path1),
                        ggdraw() + draw_image(path2, scale = 0.8),
                        labels = NULL, ncol = 2)
  plot_tmp <- plot_grid(title, plot_tmp, ncol = 1, rel_heights = c(0.1, 1))
  save_plot(paste0("data-analysis/figures/ncpc/taskSM-cmb-ncpc", j, ".jpg"), plot_tmp,
            base_height = 5, base_aspect_ratio = 2.5)
}
